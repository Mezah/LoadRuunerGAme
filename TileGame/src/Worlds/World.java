...
public final class World {
    ...
    private final Handler handler;  // take object from handler
    private int width, height;      
    private int[][] tiles;         // this arr is the result of converting arr toakens
    private int spawnX, spawnY;   // position of player in world.txt
    private int level, score;
    String[] toakens;           // arr of char that carry all data in file
    //ENTITIES
    private EntitiyManager entitiyManager;  //take object from EntitiyManager
    // path refered to path of file world.txt
    ...
    private void loadWorld(String path) {
        ...
        toakens = file.split("\\s+");                   // to read all data in world.txt as string without spaces
        width = Utils.parseInt(toakens[0]);          
        height = Utils.parseInt(toakens[1]);
        spawnX = Utils.parseInt(toakens[2]);        // position for player in world.txt
...
//SETTER
    ...
    public void render(Graphics g) {
        ...
        int xStart = (int) Math.max(0, handler.getGameCamera().getxOffset() / Tile.TILEWIDTH), //to more perfect "this program you can't guss which device can play in "its screen width , its screen height etc..""
        ...
        for (int y = yStart; y < yEnd; y++) {
            for (int x = xStart; x < xEnd; x++) {
                getTile(x, y).render(g, (int) (x * Tile.TILEWIDTH - handler.getGameCamera().getxOffset()),
                        (int) (y * Tile.TILEHEIGHT - handler.getGameCamera().getyOffset()));  //load part of world in memory
            }
        }
        //  ENTITIES
